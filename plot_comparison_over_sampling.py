...
fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(15, 15))
X, y = create_dataset(n_samples=5000, weights=(0.1,0.9),n_classes=2,
                      class_sep=0.2) #class_sep小于1，使得集群更密集，分类更困难

ax_arr = ((ax1, ax2), (ax3, ax4))
from imblearn.over_sampling import SMOTE
from imblearn.under_sampling import TomekLinks
for ax, sampler in zip(ax_arr, (SMOTE(random_state=0),
                                SMOTETomek(smote=SMOTE(),tomek=TomekLinks(),random_state=0))):
    clf = make_pipeline(sampler, LinearSVC())
    clf.fit(X, y)
    plot_decision_function(X, y, clf, ax[0])
    ax[0].set_title('Decision function for {}'.format(
